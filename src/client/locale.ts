...
export const zh: Record<MailNotifyLocaleKey, string> = {
  ...
  invalidAddress: '不是有效的邮件地：{entries}',
  statusPlugin: '插：{state}',
  statusCredential: '凭：{state}',
  statusQueue: '队列 {depth}/{size} · 已投递 {delivered} · 失败 {failed}',
  statusEffectiveQuestions: '提问通知实际生：{value}',
  statusEffectiveApprovals: '批准通知实际生：{value}',
  statusConfigError: '已保存的配置无法应用，之前的设置仍然生：{message}',
  ...
  noticeResetStaged: '已暂存重：保存后所有字段将恢复为组合配置值。',
  noticeSaved: '已保存',
  noticeSaveRefused: '宿主未接受本次保：{message}',
  noticeSaveRefusedDefault: '宿主未接受本次保存。',
  noticeCredentialRemoved: '已保存的密码已删除。',
  noticeCredentialRefused: '密码更改被拒：{message}',
  testEmailSent: '测试邮件已发送。SMTP 服务器已接受该邮件，共 {count} 个收件人。',
  testEmailRefused: 'SMTP 服务器拒绝了该邮件。',
  testEmailServerMessage: 'SMTP 服务器拒绝了该邮：{message}',
  testEmailFailed: '测试邮件发送失：{message}',
}
